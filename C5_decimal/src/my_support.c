...
void mul_only_bits(my_decimal value_1, my_decimal value_2, my_decimal *result) {
  clean(result);
  my_decimal tmp_res;
...
void sub_only_bits(my_decimal value_1, my_decimal value_2, my_decimal *result) {
  clean(result);
...
void div_only_bits(my_decimal number_1, my_decimal number_2, my_decimal *buf,
                   my_decimal *result) {
  clean(buf);
  clean(result);
  for (int i = get_highest_bit(number_1); i >= 0; i--) {
    if (get_bit(number_1, i)) set_bit(buf, 0, 1);
    if (my_is_greater_or_equal(*buf, number_2) == TRUE) {
...
int sub_without_scale(my_decimal value1, my_decimal value2,
                      my_decimal *result) {
  clean(result);
  int ret = 0;
  int reminder = 0;
  if (!get_bit(value1, 127) && get_bit(value2, 127)) {
    my_negate(value2, &value2);
    ...
    if (get_bit(value1, 127) && get_bit(value2, 127)) {
      my_negate(value2, &value2);
      my_negate(value1, &value1);
      swap_values(&value1, &value2);
    }
    if (!my_is_greater_or_equal(value1, value2)) {
...
void scale_equalize(my_decimal *value_1, my_decimal *value_2) {
  my_decimal ten = {{10u, 0, 0, 0}};
...
int add_without_scale(my_decimal value_1, my_decimal value_2,
  ...
  if (get_bit(value_1, 127) != get_bit(value_2, 127)) {
    my_decimal value_1tmp = value_1;
    my_decimal value_2tmp = value_2;
    if (get_bit(value_1, 127))
      my_negate(value_1, &value_1tmp);
    else
      my_negate(value_2, &value_2tmp);
    result_Function = sub_without_scale(value_1tmp, value_2tmp, result);
    if (get_bit(value_1, 127) && my_is_greater(value_1, value_2))
      my_negate(*result, result);
    else if (my_is_greater(value_2, value_1))
      my_negate(*result, result);
  } else {
    int check_minus;
    int in_mind = 0;
...
int shift_left(my_decimal *dec, int shift) {
  ...
  if (highest_bit + shift > 95) {
    ...
    for (int i = 0; i < shift; i++) {
      ...
      if (last_low_bit_set) set_bit(dec, 32, 1);
      if (last_mid_bit_set) set_bit(dec, 64, 1);
    }
  }
  if (get_bit(*dec, 127) && ret_val) ret_val = 2;
  return ret_val;
}

int sign_minus(my_decimal a) { return a.bits[3] >> 31; }
...
int equalize_to_bigger(my_decimal *dec, int scale) {
  int ret = 0;
  while (scale--) {
    my_decimal buf1 = *dec, buf2 = *dec;
    ret = shift_left(&buf2, 3);
    if (ret != 0) break;
    ret = shift_left(&buf1, 1);
    if (ret != 0) break;
    ret = my_add(buf2, buf1, dec);
    if (ret != 0) break;
  }
  return ret;
}
...
my_decimal binary_division(my_decimal dec1, my_decimal dec2,
  ...
  *fail = processing_eq_scales(&dec1, &dec2, &result_scale);
  for (int i = get_highest_bit(dec1); i >= 0; i--) {
    if (get_bit(dec1, i)) set_bit(remainder, 0, 1);
    if (my_is_greater_or_equal(*remainder, dec2)) {
      *fail = sub_without_scale(*remainder, dec2, remainder);
      if (i != 0) *fail = shift_left(remainder, 1);
      if (get_bit(dec1, i - 1)) set_bit(remainder, 0, 1);
      *fail = shift_left(&result, 1);
      set_bit(&result, 0, 1);
    } else {
      *fail = shift_left(&result, 1);
      if (i != 0) *fail = shift_left(remainder, 1);
      if (i - 1 >= 0 && get_bit(dec1, i - 1)) set_bit(remainder, 0, 1);
    }
  }
...
int equalize_scales(my_decimal *dec1, my_decimal *dec2, int scale) {
  ...
  if (sign1) set_bit(dec1, 127, 1);
  if (sign2) set_bit(dec2, 127, 1);
  return ret;
}

int processing_eq_scales(my_decimal *value_1, my_decimal *value_2,
                         int *final_scale) {
  my_decimal value1_tmp = *value_1;
  my_decimal value2_tmp = *value_2;
...
int float_binary_exp(float *src) { return ((*(int *)src & ~SIGN) >> 23) - 127; }
